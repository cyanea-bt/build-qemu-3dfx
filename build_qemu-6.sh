...
if [[ -d "/opt/qemu-6-${opt_host_os}" ]]; then
    rm -rf /opt/qemu-6-${opt_host_os}
fi
../qemu-6-patched/configure --prefix=/opt/qemu-6-${opt_host_os} --target-list="${LIST_TARGETS}" ${EXTRA_OPTS} \
                            ...
                            --enable-spice --enable-spice-protocol --enable-lto \
